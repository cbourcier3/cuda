...
__global__ void cal_pi(long num_steps, double step, double *sum)
{
		...
		double local;
		local=0.0;
		int idx = blockIdx.x*blockDim.x+threadIdx.x;  // index du thread
		int saut = blockDim.x * gridDim.x;
	for (i=idx;i<= num_steps; i+=saut){
		x = (i-0.5)*step;
		local += 4.0/(1.0+x*x);
	}
...
//
		...
		cal_pi<<<512,512>>>(num_steps,step,dev_sum);
